...
void min_max1(int t[], int& min, int& max) { // zakladamy n >= 1
 min=max=t[0];
 for(int i=1; i<n; i++){
    if(max<t[i]) 
		max=t[i];
...
void min_max2(int left, int right, int t[], int& min, int& max) {
 if (left==right)
	...
	if (left==right-1) // dwa elementy
		...
		// min=temp_min1<temp_min2 ? temp_min1 : temp_min2; // (1) bardziej zwiezla wersja,  w stylu jezyka C
	 	// max=temp_max1>temp_max2 ? temp_max1 : temp_max2; // (2) bardziej zwiezlaa wersja, w stylu jezyka C
...
int main(){
 int min, max;
 cout  << "23, 12, 1, -5, 34, -7, 45, 2, 88, -3, -9, 1\n";
